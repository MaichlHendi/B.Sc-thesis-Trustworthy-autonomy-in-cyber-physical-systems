...
import subprocess
from patchFilter import AutoEncoder8 
#import patch_config
import os
import sys
import time
...
class FrequencyLoss(torch.nn.Module):
    def __init__(self, patch_size, low_freq_ratio=0.25):
        super(FrequencyLoss, self).__init__()
        self.patch_size = patch_size
        self.low_freq_ratio = low_freq_ratio  # e.g., 0.25 means focus on lowest 25% of frequencies

        # Create a circular low-pass mask in the frequency domain
        yy, xx = torch.meshgrid(torch.arange(patch_size), torch.arange(patch_size), indexing='ij')
        center = patch_size // 2
        dist = ((yy - center)**2 + (xx - center)**2).float().sqrt()
        radius = patch_size * low_freq_ratio
        self.low_freq_mask = (dist < radius).float()[None, :, :]  # Shape: [1, H, W]

    def forward(self, patch):
        # patch: [3, H, W]
        patch_fft = torch.fft.fftshift(torch.fft.fft2(patch, dim=(-2, -1)), dim=(-2, -1))
        mag = torch.abs(patch_fft)  # [3, H, W]
        low_freq_energy = (mag * self.low_freq_mask.to(patch.device)).sum()
        return low_freq_energy / patch.numel()  # normalize
...
def train(args):
    ...
    nutnet_ae = AutoEncoder8().cuda().eval()  # Match NutNet settings
    nutnet_ae.load_state_dict(torch.load('./ae_weights/n_52.pth'))  # Update path if needed
    nutnet_loss_fn = torch.nn.MSELoss()
    ...
    frequency_loss = FrequencyLoss(args.patch_size, low_freq_ratio=0.25).cuda()
    ...
    for epoch in range(n_epochs):
        ...
        for i_batch, (img_batch, lab_batch, pth_i) in tqdm(enumerate(train_loader), desc=f'Running epoch {epoch}',
            ...
            freq = frequency_loss(adv_patch)
            freq_loss_weight = 1.0  # You can tune this value
            ...
            # NutNet-aware patch loss
            reconstructed_patch = nutnet_ae(adv_patch.unsqueeze(0))  # [1,3,H,W]
            recon_loss_nutnet = nutnet_loss_fn(reconstructed_patch, adv_patch.unsqueeze(0))

            recon_loss_weight = 5.0  # You can tune this
            loss = loss + recon_loss_weight * recon_loss_nutnet
